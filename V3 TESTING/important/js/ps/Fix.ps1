param(
    [string]$ProjectRoot = "C:\Users\$env:USERNAME\Downloads\V3 TESTING",
    ...
    [string]$SourceSubPath = 'V3 TESTING',
...
if (Test-Path $ConfigDir) {
    ...
    function Upsert-Line([string]$content, [string]$pattern, [string]$replacement) {
    ...
    if ($EffectiveEndpoint) { $newContent = Upsert-Line $newContent "window\.__MS_ENDPOINT__\s*=\s*'[^']*';" "window.__MS_ENDPOINT__ = '$EffectiveEndpoint';" }
    if ($EffectiveVCEndpoint) { $newContent = Upsert-Line $newContent "window\.__VC_ENDPOINT__\s*=\s*'[^']*';" "window.__VC_ENDPOINT__ = '$EffectiveVCEndpoint';" }
